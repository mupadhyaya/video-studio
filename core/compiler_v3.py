...
from moviepy import ImageClip, AudioFileClip, VideoFileClip, concatenate_videoclips, ImageSequenceClip, VideoClip
from moviepy.video.compositing.CompositeVideoClip import CompositeVideoClip
from moviepy.video.fx import CrossFadeIn
from core.avatar_ml import synthesize_talking_face
from PIL import Image, ImageDraw, ImageFont
from core.image_engine import get_font, wrap_text

def apply_dynamic_typing_to_content(content_clip, visual_type, text_content, width, height):
    duration = content_clip.duration
    base_frame = content_clip.get_frame(0)
    base_img = Image.fromarray(base_frame).convert("RGB")
    ...
    if visual_type == "terminal_output":
        text_content = text_content.replace("```bash", "").replace("```sh", "").replace("```", "").strip()
        
    lines = text_content.split('\n')
    wrapped_lines = []
    for line in lines:
        if visual_type == "terminal_output" and not line.strip():
            wrapped_lines.append("")
            continue
        wrapped_lines.extend(wrap_text(line, font, box_w - 40))
    ...
    def make_frame(t):
        ...
        # Erase existing text (leave a small margin for rounded corners)
        draw.rectangle([box_x + 5, box_y + header_h + 5, box_x + box_w - 5, box_y + box_h - 10], fill=bg_color)
        ...
        for line in wrapped_lines:
            ...
            line_len = len(line)
            if chars_drawn + line_len <= chars_to_show:
                color = text_color
                if visual_type == "terminal_output":
                    if line.strip().startswith("$") or line.strip().startswith(">"):
                        color = "#4ADE80"
                draw.text((box_x + 20, text_y), line, font=font, fill=color)
                chars_drawn += line_len
            else:
                remaining = chars_to_show - chars_drawn
                if remaining > 0:
                    partial = line[:remaining]
                    color = text_color
                    if visual_type == "terminal_output":
                        if partial.strip().startswith("$") or partial.strip().startswith(">"):
                            color = "#4ADE80"
                    draw.text((box_x + 20, text_y), partial, font=font, fill=color)
                break
            text_y += line_spacing
            
        return np.array(frame_img)

    return VideoClip(frame_function=make_frame, duration=duration).with_mask(content_clip.mask)
...
def compile_video(slides_data, images_dir, audio_dir, output_path):
    ...
    try:
        for i, slide in enumerate(slides_data):
            ...
            visual_type = slide.get("visual_type", "")
            if visual_type in ["code_snippet", "terminal_output"]:
                print(f"  [V3 EFFECT] Generating Dynamic Typing for slide {i}...")
                content_clip = apply_dynamic_typing_to_content(
                    content_clip,
                    visual_type,
                    slide.get("visual_content", ""),
                    1920, 1080
                )
            
            slide_clip = CompositeVideoClip([base_clip, content_clip])
            
            composite_layers = [slide_clip]
            
            # --- 2. ML Lip-sync Generation ---
            use_ml_avatar = False  # Disabled for fast v3 testing
